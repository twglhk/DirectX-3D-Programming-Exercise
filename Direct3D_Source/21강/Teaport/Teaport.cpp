...
D3DXVECTOR3 g_vEye, g_vAt, g_vUp;
                                     ...
                                     void* pUserContext )
{
	...
	g_vAt.x = g_vAt.y = g_vAt.z = 0.0f;
	g_vUp.x = 0.0f;
	g_vUp.y = 1.0f;
	g_vUp.z = 0.0f;

	D3DXMatrixLookAtLH( &g_matView, &g_vEye, &g_vAt, &g_vUp );
	pd3dDevice->SetTransform( D3DTS_VIEW, &g_matView );

	D3DXMatrixPerspectiveFovLH( &g_matProj, D3DX_PI /4.0f, 1.0f, 1.0f, 100.0f );
	pd3dDevice->SetTransform( D3DTS_PROJECTION, &g_matProj );
	pd3dDevice->SetViewport( &vp );

	D3DXCreateTeapot( pd3dDevice, &g_pTeapotMesh, NULL );
