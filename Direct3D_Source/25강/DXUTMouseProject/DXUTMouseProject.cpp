                          ...
                          bool* pbNoFurtherProcessing, void* pUserContext )
{
	if( uMsg == WM_MOUSEMOVE )
	{
		...
		swprintf(g_strText, L"x : %d y : %d ", LOWORD(lParam), HIWORD(lParam));
	}

	if( uMsg == WM_LBUTTONDOWN )
	{
		lstrcat(g_strText, L" 왼쪽 클릭");
	}

	if( uMsg == WM_RBUTTONDOWN )
	{
		lstrcat(g_strText, L" 오른쪽 클릭");
	}

	if( uMsg == WM_LBUTTONDBLCLK )
	{
		lstrcat(g_strText, L" 더블클릭");
	}
...
{
	ZeroMemory( g_strText, sizeof( g_strText ));
	swprintf(g_strText, L"x : %d y : %d 왼쪽 클릭: %d 오른쪽 클릭: %d", xPos, yPos, 
												bLeftButtonDown, bRightButtonDown );
}
...
INT WINAPI wWinMain( HINSTANCE, HINSTANCE, LPWSTR, int )
{
    ...
    DXUTCreateWindow( L"DXUTMouseProject" );
